...
$(document).ready(function () {
    ...
    $(".thumbnail").click(function(){
        console.log('.thumbnail click : id=' + $(this).attr('id'));
        var urn = base64encode($(this).attr('id'));
        console.log('base64encode :' + urn);
        initializeViewer(urn);
    });
...
function get3LegToken(callback) {
    ...
    console.log('get3LegToken [start]');

    //window.location.href = "/api/forge/gettoken/"
    //return;
    ...
    if (callback) {
        $.ajax({
            ...
            success: function (data) {
                console.log('get3LegToken [success]');
                MyVars.token3Leg = data.access_token;
                console.log('get3LegToken : Returning new 3 legged token (User Authorization): ' + MyVars.token3Leg);
                callback(data.access_token, data.expires_in);
            },
            error: function (data) {
                console.log('get3LegToken [error]');
            }
        });
    } else {
        console.log('get3LegToken : Returning saved 3 legged token (User Authorization): ' + MyVars.token3Leg);
...
function downloadDerivative(urn, derUrn, fileName) {
    console.log('downloadDerivative [start]');
...
function getThumbnail(urn) {
    console.log('getThumbnail [start]');
    console.log("getThumbnail for urn=" + urn);
    // fileName = file name you want to use for download
    var url = window.location.protocol + "//" + window.location.host +
...
function prepareFilesTree() {
    console.log("prepareFilesTree [start]");
    $('#forgeFiles').jstree({
        'core': {
            'themes': {"icons": true},
        ...
        console.log("prepareFilesTree [bind]");
...
function filesTreeContextMenu(node, callback) {
    console.log("filesTreeContextMenu [start]");
...
function showHierarchy(urn, guid, objectIds, rootFileName, fileExtType) {
    ...
    console.log("showHierarchy [start]");
    // You need to
    // 1) Post a job
...
function addHierarchy(nodes) {
    ...
    console.log("addHierarchy [start]");
...
function prepareHierarchyTree(urn, guid, json) {
    ...
    console.log("prepareHierarchyTree [start]");
    ...
    $('#forgeHierarchy').jstree({
        ...
        console.log("prepareHierarchyTree [bind] select_node.jstree");
        ...
        console.log("prepareHierarchyTree [bind] check_node.jstree uncheck_node.jstree");
...
function selectInHierarchyTree(objectIds) {
    ...
    console.log("selectInHierarchyTree [start]");
...
function hierarchyTreeContextMenu(node, callback) {
    ...
    console.log("hierarchyTreeContextMenu [start]");
    var menuItems = {};
...
function preparePropertyTree(urn, guid, objectId, props) {
    ...
    console.log("preparePropertyTree [start]");
    // Convert data to expected format
...
function cleanupViewer() {
    ...
    console.log("cleanupViewer [start]");
    // Clean up previous instance
    if (MyVars.viewer && MyVars.viewer.model) {
        console.log("cleanupViewer : Unloading current model from Autodesk Viewer");
...
function initializeViewer(urn) {
    ...
    console.log("initializeViewer [start]");
    cleanupViewer();
    ...
    console.log("initializeViewer [end]");
}

function addSelectionListener(viewer) {

    console.log("addSelectionListener [start]");
...
function loadDocument(viewer, documentId) {
    ...
    console.log("loadDocument [start]");
...
function selectInViewer(objectIds) {
    ...
    console.log("selectInViewer [start]");
    if (MyVars.viewer) {
...
function showProgress(text, status) {
    ...
    console.log("showProgress [start]");
